...
names = ["quality","heartrate","attention","meditation","8bit_raw","eeg_raw","eeg"]
    ...
    def fetch_data(self):
        ...
        while i < len(payload)-1:
            ...
            if code in codes: #check if current byte is a supported code
                ...
                self.values[names[c]] = payload[i+1] if c < 5 \
                ...
                if c < 5: #all single-byte codes (quality, heartrate, attention, meditation, 8bit_raw)
                    self.values[names[c]] = payload[i+1]
                elif c == 5: #eeg_raw (fun fact: the first byte after the code is completely useless)
                    self.values[names[c]] = signed_thing_to_int(payload[i+2] << 8 | payload[i+3], 16)
                elif c == 6: #eeg
                    self.values[names[c]] = dict(zip(bands, [payload[b] << 16 | payload[b+1] << 8 | payload[b+2] for b in range(i+1, i+25, 3)]))
...
while True:
    try:
        ...
        if len(eeg_device.values) == 1: y_vals = y_vals[1:]+[int(size[1]/2-size[1]*eeg_device.values["eeg_raw"]/raw_eeg_range)]
